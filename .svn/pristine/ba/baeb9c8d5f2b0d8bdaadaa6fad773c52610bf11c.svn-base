...
ASPELL(spell_teleport)
{
    ...
    } while (IS_SET_AR(world[to_room].room_flags, ROOM_CLAN | ROOM_PRIVATE | ROOM_DEATH | ROOM_GODROOM) ||
             ZONE_FLAGGED(world[to_room].zone, ZONE_NOPORTAL));
...
ASPELL(spell_summon)
{
  ...
  if( victim->mount != NULL && !PLR_FLAGGED(victim, PLR_HUNTED))
  {
  ...
  /* If we get this far with a mount, it's because the victim is
     a mounted HUNTED player.  Make them fall awkwardly.         */
  if (victim->mount)
...
//128    sprintbit(GET_OBJ_EXTRA(obj), extra_bits, buf);
    ...
    if(IS_OBJ_STAT(obj, ITEM_TROPHY)) {
        sendChar(ch, "Expires on: %d%s day of the %d%s month, %d.\r\n",
                (GET_OBJ_TIMER(obj)%TICKS_PER_MUD_MONTH)/TICKS_PER_MUD_DAY, 
                ndth((GET_OBJ_TIMER(obj)%TICKS_PER_MUD_MONTH)/TICKS_PER_MUD_DAY ),
                (GET_OBJ_TIMER(obj)%TICKS_PER_MUD_YEAR)/TICKS_PER_MUD_MONTH , 
                ndth((GET_OBJ_TIMER(obj)%TICKS_PER_MUD_YEAR)/TICKS_PER_MUD_MONTH),
                GET_OBJ_TIMER(obj)/TICKS_PER_MUD_YEAR);
    }
...
ASPELL(spell_charm_corpse)
{
    ...
    int castFail = 0;
    for (afTemp = ch->affected; afTemp; afTemp = afTemp->next) {
        if(afTemp->type == SPELL_CHARM_CORPSE2 && percentSuccess(5))
                castFail = 1;
    }
    
    /* Bigger it is, harder it is.  We return the corpse to the floor when this fails.*/
    if (castFail || percentSuccess(GET_MAX_HIT(mob)/(GET_LEVEL(ch)*4))) {
...
// Normalize the damage a mob does
    ...
    mob->decay += number(GET_MAX_HIT(mob)/1000, GET_MAX_HIT(mob)/400) *
        (affected_by_spell(mob, SPELL_SANCTUARY)? 7 : affected_by_spell(mob, SPELL_WARD)? 5 : 4)/4;
    // Stronger mobs decay even more readily
    mob->decay += number(raw_damage/80, raw_damage/25);
    ...
    add_affect(ch, ch,  SPELL_CHARM_CORPSE2, 0, 0, 0, SECS_PER_MUD_HOUR*3/2, 0, FALSE, FALSE, FALSE, FALSE);
    ...
    if(IS_WITCH_DOCTOR(ch)) {
        ...
        if(GET_ADVANCE_LEVEL(ch) >= THIRD_ADVANCE_SKILL) {
            GET_MOB_SPELL_DAM(mob) += 15;
            if(GET_MOB_SKILL_SUC(mob) < 90)
                GET_MOB_SKILL_SUC(mob) = MIN(GET_MOB_SKILL_SUC(mob) + 15, 90);
        }
    }
		...
		if (!CAN_WEAR(obj, ITEM_WEAR_TAKE)) {
			...
			return;
                ...
                if(ROOM_FLAGGED(IN_ROOM(ch), ROOM_CLAN)) {
                    send_to_char("Clan halls are a sacred place; entombing here may upset the natural balance.\r\n", ch);
                    return;
                }
...
ASPELL(spell_summon_corpse)
{
    ...
    if(!IS_NPC(victim) && !victim->desc) {
        sendChar(ch, "The gods forbid summoning the corpse of a linkless player.\r\n");
        return;
    }
...
char *spells[TOP_SPELL_DEFINE + 2] =
{
  ...
  "unimpeded asault", "defender health", "zombie reward", "charm corpse2", "!UNUSED!",	/* 340 */
...
const char *spell_wear_off_msg[NUM_SPELLS] = {
  ...
  "The deep burns on your skin become less sensitive.",
  "!tremor!",                            /* 150 */
  "!tsunami!",
  "!typhoon!",
  "!terror!",
