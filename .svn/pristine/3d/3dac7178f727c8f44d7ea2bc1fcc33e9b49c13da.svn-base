...
int bindItem(CharData *ch, CharData *identifier, ObjData *obj) {
  char logBuf[500];

  ObjData *identify_obj = NULL;
  identify_obj = read_perfect_object(GET_OBJ_VNUM(obj), VIRTUAL);
  if (!identify_obj) {
    send_to_char("This object cannot be bound!\r\n", ch);
    mlog("Can't create virtual object for Elie to bind, object doesn't exist");
    return FALSE;
  }

  if (PLR_FLAGGED(ch, PLR_HUNTED)) {
    act("$n tells you, 'Leave the premises, you lowlife scum!",  FALSE, identifier, 0, ch, TO_VICT);
    return FALSE;
  }

  if(IS_OBJ_STAT(obj, ITEM_SOULBOUND)) {
    act("$n tells you, 'This item is already bound to you.",  FALSE, identifier, 0, ch, TO_VICT);
    return FALSE;
  }

  if(GET_QP(ch) < bindRate) {
    act("$n tells you, 'You need the favor of the gods for this process.  Return with quest points.",  FALSE, identifier, 0, ch, TO_VICT);
    return FALSE;
  }
  else {
    GET_QP(ch) -= bindRate;
  ...
  sprintf(logBuf, "%s binded the item %s (vnum %d) .", GET_NAME(ch), obj->short_description, GET_OBJ_VNUM(obj));
  general_log(logBuf, "misc/rerolls");
  act("$n takes $p and looks it over; he then hands it back to you.  You feel a special connection to it.",  FALSE, identifier, obj, ch, TO_VICT);
...
"|               |                                        |      %d Gold |\r\n"
"|               |                                        |                 |\r\n"
"| Bind          | Permanently link an item to yourself.  |                 |\r\n"
"|               | The gods restrict you from giving this |                 |\r\n"
"|               | item to anyone else ever again.        |                 |\r\n"
"|               |                                        |      %d QPs     |\r\n"
"|               |                                        |                 |\r\n"
"+--------------------------------------------------------------------------+\r\n", identifyRate, discernRate, bindRate);
        sendChar( ch, "\r\n" );
        return( TRUE );
    }
