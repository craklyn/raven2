...
/* affect_remove : TRUE is for saving, it unaffects all spells when saving, this
    ...
    */
    ...
    if (killer && IS_NPC(killer) && (IS_AFFECTED(killer, AFF_CHARM) || affected_by_spell(killer, SKILL_BEFRIEND)))
    ...
    */
    for(f = ch->followers; f; f = f->next) {
        if(IS_AFFECTED(f->follower, AFF_GROUP) && roomNum == f->follower->in_room
                || IS_NPC(f->follower) && (IS_AFFECTED(f->follower, AFF_CHARM) || affected_by_spell(f->follower, SKILL_BEFRIEND)))
	{
            tot_members++;
		...
		high_member = f->follower;
            ...
            for(g = f->follower->followers; g; g = g->next) {
                if(IS_NPC(g->follower) && affected_by_spell(g->follower, SKILL_BEFRIEND)) {
                    tot_members++;
                    tot_levels += GET_LEVEL(g->follower);
                }
                else if(IS_NPC(g->follower) && IS_AFFECTED(g->follower, AFF_CHARM)) {
                    tot_members++;
...
void ind_gain(CharData *ch, CharData *victim, int pkill)
{
    ...
    if( affected_by_spell(victim, SPELL_CHARM_CORPSE) || IS_AFFECTED(victim, AFF_CHARM) || affected_by_spell(victim, SKILL_BEFRIEND))
        exp = 1;
			  ...
			  int dam, int type)
{
    ...
    if (affected_by_spell(victim, SKILL_NM_SACRIFICE)) {
        ...
        // Check if the player has any charmed mobs following.  If so, then exit.
        FollowType *f;
        for(f = victim->followers; f; f = f->next) {
          if(IS_NPC(f->follower) && IS_AFFECTED(f->follower, AFF_CHARM)) {
            SACRIFICE(victim) = 0;
          }
        }
        ...
        sprintf(buf, "%s's &07dark aura&00 reacts to protect %s from harm.",
...
int bonusPetDamage(CharData *ch, CharData *victim, int dam, int attack_type) {
	...
	if(affected_by_spell(ch, SPELL_CALL_OF_THE_WILD) && ch->master) {
		if(IS_HUNTER(ch->master)) {
			...
			act("&02$n lets loose a ferocious attack on $N!&00", FALSE, ch, 0, victim, TO_NOTVICT);
			sendChar(ch, "&02You let loose a ferocious attack against %s!&00\r\n", GET_NAME(victim));
...
setup_combat( CharData *ch, CharData *victim)
{
    ...
    if( GET_POS(ch) > POS_STUNNED )
    {
      ...
      if( IS_NPC(ch) && IS_NPC(victim) && victim->master &&
         !number(0, 100) && (IS_AFFECTED(victim, AFF_CHARM) || affected_by_spell(victim, SKILL_BEFRIEND))&&
      ...
      if( FIGHTING(victim) && (IS_AFFECTED(victim, AFF_CHARM) || affected_by_spell(victim, SKILL_BEFRIEND)) &&
...
damage( CharData *ch, CharData *victim, int dam, int attacktype )
{
  ...
  */
  ...
  if (dam > 0 && (IS_AFFECTED(ch, AFF_FLAME_BLADE) || IS_ELEMENTAL(ch) && GET_SUBRACE(ch) == FIRE_ELEMENTAL)) {
		// make sure the damage is a weapon type, not a skill or spell
  ...
  if (IS_AFFECTED(victim, AFF_SHIELDBLOCK) ) {
    if( IS_NPC(victim) || FIGHTING(victim) == ch) {
      ...
      if(!IS_NPC(victim) || percentSuccess(15)) {
        ...
        // Advanced players may block a second attack sometimes.
        if(FIGHTING(victim) == ch && 
           (IS_DRAGOON(victim) || IS_KNIGHT_ERRANT(victim)) &&
            GET_ADVANCE_LEVEL(victim) >= THIRD_ADVANCE_SKILL && percentSuccess(20)) {
