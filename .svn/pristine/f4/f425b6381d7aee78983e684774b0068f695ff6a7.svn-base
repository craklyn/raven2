...
#define LOCKER_UPGRADE_1        2

#define LOCKER_MAX        3

#define VNUM_START      41301
#define VNUM_MAX        41497
...
{
  {LOCKER_PURCHASE_10, "small", "  ", "Buy a 10-item locker        ", 40000000},
  {LOCKER_PURCHASE_20, "large", "  ", "Buy a 20-item locker        ", 52500000},
  {LOCKER_UPGRADE_1 ,  "upgrade", "", "Increase capacity by 1 item ",  1500000},
};
...
static int create_new_locker(CharData *ch, int size)
{
  ...
  OLC_ZNUM(ch->desc) = real_zone(GET_OBJ_VNUM(obj)/100);
  oSaveToDisk(ch->desc);
  free(ch->desc->olc);
	...
	if ( choice >= 0 )
	  {
	    ...
	    switch ( choice )
	      ...
	      case LOCKER_PURCHASE_20:
                ...
                break;
	      case LOCKER_UPGRADE_1:
                if (GET_LOCKER(ch) == 0) {
                  act("$n tells you, 'You don't own a locker!'", FALSE, manager, 0, ch, TO_VICT);
                  return TRUE;
                }
                ObjData *obj = find_locker(GET_LOCKER(ch));
                if (!obj) return -1;

                switch (update_locker_size(ch, GET_OBJ_VAL(obj, 0) + 1)) {
                  case -1:
