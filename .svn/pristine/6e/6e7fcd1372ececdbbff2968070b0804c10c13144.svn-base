    ...
    for (ch = character_list; ch; ch = next_ch) {
        ...
        if (affected_by_spell(ch, SKILL_BEFRIEND)) {
            if (!FIGHTING(ch)) {
                ...
                if (ch->master && FIGHTING(ch->master) && ch->master->in_room == ch->in_room &&
                    GET_MAX_HIT(ch->master)*5 > GET_MAX_HIT(ch)) {
        ...
        if (MOB_FLAGGED(ch, MOB_HELPER)) {
            ...
            for (vict = world[ch->in_room].people; vict && !found; vict = vict->next_in_room) {
                ...
                if (IS_NPC(vict) && vict != ch && FIGHTING(vict) && CAN_SEE(ch, FIGHTING(vict)) &&
                    ...
                    if ((class == CLASS_ASSASSIN) && has_back_wpn && !(FIGHTING(vict)->mount))
                        do_circle(ch, FIGHTING(vict)->player.name, 0, 0);
                    else if ((class == CLASS_THIEF) && (level >= 5) && has_back_wpn && !(FIGHTING(vict)->mount))
                        do_circle(ch, FIGHTING(vict)->player.name, 0, 0);
                    else
