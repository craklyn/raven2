...
magic_savingthrow (CharData *caster, CharData *victim, int type)
{
  ...
  if (IS_AFFECTED (caster, AFF_FEEBLE) && number (0, 1))
      return TRUE;

  // focused mages will often succeed
  if (skillSuccess (caster, SKILL_FOCUS) && number (1, 4) == 1)
      return FALSE;
  ...
  /* Wraithform makes you harder to resist ONLY if you are a necro */
  if (IS_AFFECTED (caster, AFF_WRAITHFORM) &&
       GET_CLASS (caster) == CLASS_NECROMANCER)
    save += aff_level (caster, AFF_WRAITHFORM) / 5 + 2;

  /* And makes you better at resisting ONLY if you are a necro */
...
if (affected_by_spell(victim, spellnum)) { send_to_char("Nothing seems to happen.\r\n", ch); return; }
    ...
    case SPELL_BARKSKIN:
        if(affected_by_spell(ch, SPELL_BARKSKIN)) {
          sendChar(ch, "The needle-sharp thorns retract beneath your skin.\r\n");
    ...
    case SPELL_REGENERATE:
        ...
        add_affect(ch, victim, spellnum, GET_LEVEL(ch), NO_APPLY, 0, 60, AFF_REGENERATE, FALSE, FALSE, FALSE, FALSE);
	    ...
	    && (ch->master))
	{
        ...
        switch (spellnum) {
            ...
            case SPELL_METEOR_SWARM:
                ...
                break;
            case SPELL_WALL_OF_FIRE:
                if(IS_ELEMENTAL(ch) && GET_SUBRACE(ch) == FIRE_ELEMENTAL)
                    add_affect(ch, tch, spellnum, level, APPLY_SAVING_SPELL, +4, 20, 0, FALSE, FALSE, FALSE, FALSE);
                mag_damage(level, ch, tch, spellnum, 1);
                break;
		      ...
		      int spellnum, int savetype)
{
  ...
  summons gate_II_summons[] = {
    ...
    */
    ...
    f = ch->followers;
    while( f ){
        if((f->follower->master == ch) && (IS_AFFECTED(f->follower, AFF_CHARM) || affected_by_spell(f->follower, SKILL_BEFRIEND))){
            mag_followers += 1;
    ...
    {
        if(pet && affected_by_spell(pet, spellnum))
        {
            act("$n dissapears into a swirling mist.", TRUE, pet, 0, 0, TO_ROOM);
            char_from_room(pet);
            char_to_room(pet, ch->in_room);
            act("$n appears out of a swirling mist.", TRUE, pet, 0, 0, TO_ROOM);
		     ...
		     int spellnum, int savetype)
{
  ...
  // Each point of APPLY_HEAL_AFFECT gives a 0.1% bonus to healing.
  int equip_healing_bonus = 0;
  int i, j;
  for(j = 0; j < NUM_WEARS; j++) {
    for(i = 0; i < MAX_OBJ_AFFECT; i++) {
      if (ch->equipment[j] && (ch->equipment[j]->affected[i].location == APPLY_HEAL_AFFECT)) {
        equip_healing_bonus += ch->equipment[j]->affected[i].modifier;
      }
    }
  }
  hit += hit*equip_healing_bonus/1000;

  // S-Elves heal and refresh with a +10% bonus.
  ...
  if(affected_by_spell(ch, SKILL_SHADOWFORM)) {
    act("You leave the shadow form.", FALSE, ch, NULL, NULL, TO_CHAR);
    act("The menacing cloud surrounding $n disperses.", TRUE, ch, NULL, NULL, TO_ROOM);
