...
// Mortius 110906: There are calls to this function that require it to return
// a value.  Setup as void it expects no value to need returned.  Changed to
// int function instead.
// void greet_memory_mtrigger(char_data *actor)

int greet_memory_mtrigger(char_data *actor)
{
  trig_data *t;
  char_data *ch;
  ...
  if(!actor)
  {
      mudlog(BRF, LVL_IMMORT, TRUE, "The mount code would have crashed us here, but Arbaces averted the problem!");
      return TRUE;
  }
...
int entry_memory_mtrigger(char_data *ch)
{
  trig_data *t;
  char_data *actor;
  struct script_memory *mem;
    ...
    if (SCRIPT_CHECK(ch, MTRIG_COMMAND) && !AFF_FLAGGED(ch, AFF_CHARM) &&
      ...
      for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
        ...
        if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
          mudlog( BRF, LVL_IMMORT, TRUE, "TRIGERR: Command Trigger #%d has no text argument!",
            GET_TRIG_VNUM(t));
          continue;
        }
  ...
  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch_next)
  {
    ...
    if (SCRIPT_CHECK(ch, MTRIG_SPEECH) && AWAKE(ch) &&
      ...
      for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
        ...
        if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
          mudlog( BRF, LVL_IMMORT, TRUE,"TRIGERR: Speech Trigger #%d has no text argument!",
            GET_TRIG_VNUM(t));
          continue;
        }
		  ...
		  obj_data *target, char *arg)
{
  ...
  if (SCRIPT_CHECK(ch, MTRIG_ACT) && !AFF_FLAGGED(ch, AFF_CHARM) &&
    ...
    for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next)  {
      ...
      if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
        mudlog( BRF, LVL_IMMORT, TRUE,"TRIGERR: Act Trigger #%d has no text argument!",
          GET_TRIG_VNUM(t));
        continue;
      }
	...
	if (target)
	  ADD_UID_VAR(buf, t, target, "target", 0);
        if (str)
          add_var(&GET_TRIG_VARS(t), "str", str, 0);
	if (arg) {
	  skip_spaces(&arg);
  ...
  if (obj && SCRIPT_CHECK(obj, OTRIG_COMMAND))
    for (t = TRIGGERS(SCRIPT(obj)); t; t = t->next) {
      ...
      if (IS_SET(GET_TRIG_NARG(t), type) &&
        ...
        mudlog( BRF, LVL_IMMORT, TRUE,"TRIGERR: O-Command Trigger #%d has no text argument!",
          GET_TRIG_VNUM(t));
        continue;
      }
...
/* FIXME: receiving boot up error SYSERR: Illegal value(s) passed to obj_to_room
 * the exception appears to be pointing to this function call. -Xiuh 03.20.09 */
...
int command_wtrigger(char_data *actor, char *cmd, char *argument)
{
  ...
  for (t = TRIGGERS(SCRIPT(room)); t; t = t->next) {
    ...
    if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
      mudlog( BRF, LVL_IMMORT, TRUE,"TRIGERR: W-Command Trigger #%d has no text argument!",
        GET_TRIG_VNUM(t));
      continue;
    }
...
void speech_wtrigger(char_data *actor, char *str)
{
  ...
  for (t = TRIGGERS(SCRIPT(room)); t; t = t->next) {
    ...
    if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
      mudlog( BRF, LVL_IMMORT, TRUE,"TRIGERR: W-Speech Trigger #%d has no text argument!",
        GET_TRIG_VNUM(t));
      continue;
    }
