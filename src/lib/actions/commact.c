...
ACMD(do_gsay)
{
  ...
  if( !*argument ) {
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        if (GET_LEVEL(d->character) == LVL_IMPL && world[(d->character)->in_room].number == 1233)
...
void perform_tell(CharData *ch, CharData *vict, char *arg)
{
    ...
    if(prefers_ignore(vict, ch)) {
        act("$N is ignoring you.", FALSE, ch, 0, vict, TO_CHAR | TO_SLEEP);
        return;
    }
    ...
    if( !PLR_FLAGGED(ch, PLR_SHUNNED) ){
        ...
        for (d = descriptor_list; d; d = d->next) {
            ...
            if (GET_LEVEL(d->character) == LVL_IMPL && world[(d->character)->in_room].number == 1233) {
                sprintf(buf, "&18$n&18 tells %s, '%s'&00", GET_NAME(vict), arg);
                act( buf, FALSE, ch, 0, d->character, TO_VICT | TO_SLEEP );
            }
        }
	...
	continue;
      ...
      if( subcmd == SCMD_CLAN)
          if(GET_CLAN(ch) != GET_CLAN(i->character) &&
                  (GET_LEVEL(i->character) != LVL_IMPL || world[i->character->in_room].number != 1233) )
              continue;

      /* Kaidon added guild channel - 09/05/97 */
      if( subcmd == SCMD_GUILD && GET_CLASS(ch) != GET_CLASS(i->character)) continue;

      if(prefers_ignore(i->character, ch))
          continue;
