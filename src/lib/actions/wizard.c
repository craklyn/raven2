...
void show_healingaffect_gear(CharData *ch, char *value) {
  ObjData *obj;
  int nr, i, j, count = 0;
  int found;
  ...
  for (i = 1; i < NUM_WEARS; i++) {
    ...
    for (nr = 0; nr <= top_of_objt; nr++) {
      ...
      for (j = 0; j < MAX_OBJ_AFFECT; j++) {
        if (obj->affected[j].location == APPLY_HEAL_AFFECT)
          found += obj->affected[j].modifier;
        }
        if (found) {
          sprintf(buf, "%s%3d. [%5d] {%3d} %s\r\n", 
                        buf, ++count, GET_OBJ_VNUM(obj), found, obj->short_description);           
        }
      } 
...
ACMD(do_show)
{
  ...
  char big_buf[32*1024];
	...
	{ "hot",      LVL_IMMORT    }, { "cold",     LVL_IMMORT    },
        ...
        { "spelldam", LVL_IMMORT    }, { "healing", LVL_IMMORT     },
        { "highrestrict", LVL_IMMORT },{ "trophy",   LVL_IMMORT    }, 
        { "weapon",   LVL_IMMORT    }, { "graf",     LVL_IMMORT    },
	{ "\n", 0 }
    };
...
#define GOD_ROOMS_ZONE 2
  ...
  case 29:
    ...
    break;
  case 30:
    show_healingaffect_gear(ch, value);
    break;
  case 31:
...
ACMD(do_wizslap)
{
  ...
  {"You swing your boot down, kicking $N's scrawny butt clear across the mud.",
   ...
   "$n's boot swings down, kicking your scrawny butt clear across the mud."},
    ...
    char* wizslapObjectMsg[][2] = {
  { "You stare at $p and utter the words, 'azje al tan za'.\n$p fades from view.",
    "$n stares at $p and utters the words, 'azje al tan za'.\n$p fades from view."
  },

  {"You summon forth demons to remove $p from your sight.",
   "A hord of demons are summoned to remove $p from $n's sight."},
  
  {"You laugh as the earth swallows $p, taking it to only god knows where!",
   "$n throws $s head back and laughs as the earth swallows $p."},

  {"You reach down and grab $p, tossing it from this plane.",
   "$n's hand reaches down, grabs $p and tosses it from this plane."},

  {"You swing your boot down, kicking $p clear across the mud.",
   "$n's boot swings down, kicking $p clear across the mud."},
  };
  ...
  // Lets work out where to send the target
  do {
  ...
  if(victim) 
  {
    /* Can only be done to players lower than yourself */
    ...
    act( wizslapVictimMsg[msg_num][0], FALSE, ch, 0, victim, TO_CHAR);
    act( wizslapVictimMsg[msg_num][1], FALSE, ch, 0, victim, TO_NOTVICT);
    act( wizslapVictimMsg[msg_num][2], FALSE, ch, 0, victim, TO_VICT);
    ...
    // Target is an object
    if (object->carried_by == ch)
      obj_from_char(object);
    else if (object->in_room != NOWHERE && object->in_room == ch->in_room)
      obj_from_room(object);
    else {
        send_to_char("Failed to find object.", ch);
        return;
    }
    
    act( wizslapObjectMsg[msg_num][0], FALSE, ch, object, 0, TO_CHAR);
    act( wizslapObjectMsg[msg_num][1], FALSE, ch, object, 0, TO_NOTVICT);
    
    obj_to_room(object, to_room);
  } 
