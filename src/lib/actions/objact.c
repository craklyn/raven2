          ...
          /* && !IS_SET_AR(PLR_FLAGS(ch), PLR_HUNTED)) /* ch is not hunted */
      {
          ...
          if (!loot_ok)
          {
              ...
              if(IS_OBJ_STAT(cont, ITEM_LOOTED)) {
                  send_to_char("This corpse has already been robbed once.  The gods forbid robbing it a second time.\r\n", ch);
                  return;
              }

              // A given player can only loot one item at a time
              if(IS_HUNTED(ch)) {
                  send_to_char("The gods forbid a hunted player from robbing a second corpse.\r\n", ch);
                  return;
              }
              ...
              byte mode, char *sname, sh_int RDR)
{
  ...
  switch (mode)
  {
      ...
      case SCMD_JUNK:
          ...
          // Junking a quest stick grants 2qp
          if(GET_OBJ_VNUM(obj) == QUEST_STICK_VNUM) { 
            GET_QP(ch) = GET_QP(ch) + 2;
            send_to_char("&10You have earned 2qps!&00\r\n", ch);
            mudlog(BRF, LVL_IMMORT, TRUE, "%s has received 2 quest points as a reward for junking a quest stick.", GET_NAME(ch));
          }
          extract_obj(obj);
          if (!IS_NPC(ch))
              Crash_crashsave(ch);
