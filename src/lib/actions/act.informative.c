  ...
  for (d = descriptor_list; d; d = d->next) {
	...
	strcat(buf, " (writing)");
      if (IS_AFK(tch)) {
        ...
        if( strcmp(GET_NAME(tch), "Boraj" ) == 0 )
            strcat(buf, " (&03El Residente&00)");
        else
            strcat(buf, " (AFK)");
      }
      if (PRF_FLAGGED(tch, PRF_NOWIZ))
...
ACMD(do_users)
{
  ...
  if (1 == 1) {
    sendChar(ch, "Command 'users' disabled due to crash bug. Bug Arbaces when this becomes a problem for you. =]\r\n");
    return;
  }
