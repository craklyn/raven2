...
ACMD(do_advance_toggle) {
    ...
    switch(subcmd) {
        ...
        case SCMD_SHADOWFORM:
            if(IS_DARK_PRIEST(ch)) {
                if(affected_by_spell(ch, SKILL_SHADOWFORM)) {
                    ...
                    if(COOLDOWN(ch, SLOT_SHADOW_FORM)) {
                        act("You are not ready to enter into the shadows.", FALSE, ch, NULL, NULL, TO_CHAR);
                    }
                    else {
                        stun = 1;
                        act("You step away from the healing arts and enter the shadow form.", FALSE, ch, NULL, NULL, TO_CHAR);
        ...
        case SCMD_PHASESHIFT:
            ...
            if(COOLDOWN(ch, SLOT_PHASESHIFT)) {
                sendChar(ch, "You are not ready to phaseshift again.\r\n");
                return;
            }

            stun = 1;
            COOLDOWN(ch, SLOT_PHASESHIFT) = 5;
